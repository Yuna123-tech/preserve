...
const Quiz: React.FC = () => {
  // FIX: Destructure setQuestions from useState to allow updating the questions state.
              ...
              {currentQuestion.options.map((option, index) => {
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-xl text-2xl font-semibold shadow-md transition-all duration-300 transform disabled:cursor-not-allowed ${buttonClass}`}
                  >
                    {option}
                  </button>
                );
      ...
      case 'finished':
        return (
          <div className="text-center">
            ...
            <button
              onClick={startQuiz}
              className="mt-8 bg-orange-500 text-white font-display text-4xl px-8 py-3 rounded-full shadow-lg hover:bg-orange-600 transition-colors"
            >
              다시 풀어보기
            </button>
          </div>
      ...
      default:
        return (
          <div className="text-center">
            ...
            <button
              onClick={startQuiz}
              className="bg-red-500 text-white font-display text-5xl px-10 py-4 rounded-full shadow-lg hover:bg-red-600 transition-colors transform hover:scale-105"
            >
              퀴즈 시작하기!
            </button>
          </div>
